...
export default class Animals {
        ...
        switch (method) {
            case "image": {
                ...
                if (r.statusCode !== 200) {
                    ...
                    const error = !e ? new TypeError(`Non 200-OK status code returned from api: ${r.statusCode} ${r.statusMessage}`) : new TypeError(e);
                    throw error;
                }
                ...
                this.options.debug(`${this.options.baseURL}/animals/${cat}/image?notes=disabled${maxImageSize ? `&sizeLimit=${maxImageSize}` : ""}`, { start, end, time: parseFloat((end - start).toFixed(2)) });
            ...
            case "json": {
                ...
                this.options.debug(`${this.options.baseURL}/animals/${cat}?notes=disabled${maxImageSize ? `&sizeLimit=${maxImageSize}` : ""}`, { start, end, time: parseFloat((end - start).toFixed(2)) });
    ...
    get birb() {
        return this.sendRequest.bind(this, "birb") as typeof f;
    }
    get blep() {
        return this.sendRequest.bind(this, "blep") as typeof f;
    }
    get dikdik() {
        return this.sendRequest.bind(this, "dikdik") as typeof f;
    }
}
